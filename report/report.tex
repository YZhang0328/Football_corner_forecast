...
The value \(\alpha=0.1\) is the L2 penalty strength, selected on the tune seasons not on the validation seasons. Thus there's no data leakage.
small:
\[
...
missing values with a simple mean. The third row is the final model: it uses a
tree-model fitted cold-start prior value instead of a mean fill when the sufficient team history is missing.
...
A team's \texttt{kf\_attack} value is only recorded once \emph{both} that team
and its opponent have each completed at least 5 matches. On the training set
this affects 185 matches (370 rows, 8.9\,\% of training data) and is tolerable.
On the holdout set the problem is more serious.

Four of the 28 holdout teams---\texttt{team\_30}, \texttt{team\_34},
\texttt{team\_36}, and \texttt{team\_47}---never appear in training at all,
so they carry zero Kalman history into the holdout period.
Because the warm-up condition requires \emph{both} teams in a match to have
enough history, every match involving a new team produces two missing rows:
one for the new team and one for its opponent.
In total, \textbf{360 of the 1{,}520 holdout rows (23.7\,\%) have no
\texttt{kf\_attack} value.}
...
The prior is a supervised model for the \emph{missing} Kalman attack feature,
not a corner-count model. It is fitted only on training rows where the Kalman
filter has already produced a real pre-match attacking-strength estimate, and
it learns the mapping
\[
(\texttt{feature\_1},\ldots,\texttt{feature\_10},\;\texttt{home})
\;\longrightarrow\;
\texttt{kf\_attack}.
\]
At inference time it fills only the rows where the Kalman attack estimate is
missing. Rows with genuine Kalman history keep their original value untouched;
the prior is purely a fallback for cold-start rows. The final Poisson model
receives the same set of features for every row: the 10 anonymised match
features, the home indicator, and the attacking-strength estimate---drawn from
real Kalman history where available, and from the prior otherwise. This
preserves the historical Kalman signal wherever it exists, while giving
cold-start rows a feature-conditioned initial estimate rather than a single
...
5th--95th percentile of the observed \texttt{kf\_attack} distribution from
training rows with real Kalman history. This prevents the prior from assigning
implausibly large or small values for feature combinations it has not seen.
The prior can still distinguish stronger from weaker cold-start rows within
that range; it simply cannot extrapolate to an extreme initial strength
unsupported by any historical Kalman state.
...
\texttt{kf\_attack}; it also requires the features to be on comparable scales,
which cannot be verified for anonymous inputs.
...
then fitted on those same rows and applied to fill missing \texttt{kf\_attack}
values in the holdout. The prior is therefore used only as a targeted fallback
for missing team-history features, not as a replacement for either the Kalman
filter or the final corner model.
...
features. These are cruder versions of \texttt{kf\_attack}: the Kalman filter
already summarises each team's attacking history, updating that estimate match
by match with uncertainty tracking. The static mean ignores recency and
carries no uncertainty signal. In ablation it added nothing after
\texttt{kf\_attack} was already in the model.

\paragraph{Rolling form features.}
Short-window rolling means (last 3, 5, and 10 matches) overlap substantially
with what \texttt{kf\_attack} already captures. The Kalman filter is itself a
weighted average of past observations, emphasising recent matches through its
uncertainty dynamics. Adding rolling means alongside it therefore introduces
near-duplicate columns. The tune-period ablation showed no consistent gain from
any rolling window, alone or combined with \texttt{kf\_attack}.

\paragraph{Team, opponent, and season identity features.}
Team and opponent identities are already encoded through \texttt{kf\_attack}.
Adding them as categorical dummy variables asks the model to re-learn a static
per-team offset that the Kalman feature already provides dynamically. Season
identity produced a scale problem: the raw year values (2012--2022) are an
order of magnitude larger than the other features, which destabilised the
...
\texttt{kf\_attack}, the residual home/away drift signal was too small to
recover reliably in the tune period.
